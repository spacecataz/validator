...
class TestBinaryEventTable(unittest.TestCase):
    ...
    # Calculations:
    knownCalcs1 = {'s': 2./3., 'r': 4/12, 'bias': .5,  'ar': 2+2/3,
                   'dr': 2+2/3, 'PC': .5, 'HR': .375, 'FARate': .25, 'HSS': .1}
    knownCalcs2 = {'s': .97222, 'r': .9444, 'bias': .971, 'ar': 66.111,
                   'dr': .111111, 'PC': .9722, 'HR': .9714, 'FARate': 0.0,
                   'HSS': .65384}
    ...
    def test_calcB(self):
        self.assertEqual(self.knownCalcs1['bias'], self.t1.calc_bias())
        self.assertAlmostEqual(
            self.knownCalcs2['bias'], self.t2.calc_bias(), 3)
    ...
    def test_calcFARate(self):
        self.assertEqual(self.knownCalcs1['FARate'],
                         self.t1.calc_FARate())
        self.assertAlmostEqual(self.knownCalcs2['FARate'],
                               self.t2.calc_FARate())

    def test_calcHSS(self):
        self.assertAlmostEqual(self.knownCalcs1['HSS'], self.t1.calc_heidke())
        self.assertAlmostEqual(
            self.knownCalcs2['HSS'], self.t2.calc_heidke(), 4)
